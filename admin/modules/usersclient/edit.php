...
if (!empty($body['id'])) {
    ...
    $userDetai = firstRaw("SELECT * FROM usersclient WHERE id='$userIDDetai'");
...
if (isPost()) {
    ...
    if (empty(trim($body['email']))) {
        ...
        $email = $body['email'];
        $sql = "SELECT id FROM usersclient WHERE email='$email' AND id<>".$userIDDetai;
        if (getRows($sql) > 0) {
    ...
    if (empty($errors)) {
        ...
        if (!empty(trim($body['password']))) {
            $updateStatus['password'] = password_hash($body['password'], PASSWORD_DEFAULT);
        };

        $updateStatus = update('usersclient', $updateStatus, "id=$userIDDetai");
        ...
        if ($updateStatus) {
            ...
            setFlashData('msgType', 'danger');
            redirect('admin?module=usersclient&action=edit&id=' . $userIDDetai);
        }
    } else {
        //Có lỗi xảy ra
            ...
            <div class="col-xl-6 ">
                <div class="card mb-4">
                    <div class="card-header d-flex justify-content-between align-items-center">
                        <h5 class="card-title mb-0">Cập người dùng</h5>
                    </div>
                    <div class="card-body">
                        <form action="" method="post">
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group input-group-merge">
                                    <input type="text" id="basic-default-email" class="form-control" placeholder="Email address" aria-label="john.doe" name="email" value="<?php echo old('email', $old); ?>" aria-describedby="basic-default-email2" />
                            ...
                            <div class="mb-3">
                                ...
                                <input type="password" class="form-control" id="basic-default-username" name="password" value="" placeholder="Mật khẩu" />
