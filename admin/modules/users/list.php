...
$checkView= checkPermission($permissionsData,'users','view');
$checkAdd = checkPermission($permissionsData,'users','add');
$checkEdit = checkPermission($permissionsData,'users','edit');
$checkDelete = checkPermission($permissionsData,'users','delete');
...
if(isGet()){
    ...
    if(!empty($body['groupID'])){
        ...
        $filter  .= "$operator groupID=$groupID";
    }
...
$allUserNumber = getRows("SELECT id FROM users $filter");
...
$listAllUser = getRaw("SELECT users.id,users.name,image,email,status,activeToken,users.createAt,groups.name
 as group_name FROM users INNER JOIN `groups` ON users.groupID=`groups`.id $filter ORDER BY users.createAt ASC LIMIT $offset,$perPage");
...
<div class="content-wrapper">
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
            ...
            ?>
            <div class="col-lg-2">
                <a href="<?php echo getLinkAdmin($module,'add')?>" class="btn btn-primary">Thêm tài khoản</a>
            </div>
            <?php
            endif;
            ?>
            ...
            <div class="col-lg-2 ">
                <div class="me-2">
                    <select class="form-select " id="exampleFormControlSelect1" aria-label="Default select example" name="groupID">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="avatar avatar-online">
                                        <img src="<?php echo _WEB_HOST_ADMIN_TEMPLATE.'/assets/img/avatars/'.$item['image'] ?>" alt="" class="w-px-40 h-auto rounded-circle">
                                    </div>
                                </td>
                                <td>
                                    <?php echo $item['email']; ?>
                                ...
                                <td>
                                    <?php echo '<span class="badge bg-info">'.$item['group_name'].'</span>';?>
                                </td>
                                <td>
                                    <?php echo ($item['activeToken']==null)?'<span class="badge bg-danger">Chưa kích hoạt</span>':'<span class="badge bg-success">Đã kích hoạt</span>';?>
                                </td>
                                <td>
                                    <?php echo ($item['status']==0)?'<span class="badge bg-danger">Đang khóa</span>':'<span class="badge bg-info">Đang mở</span>';?>
                                </td>
...
?>
    <script>
        var url = '?module=users&action=delete';
