...
$checkView= checkPermission($permissionsData,'groups','view');
$checkAdd = checkPermission($permissionsData,'groups','add');
$checkEdit = checkPermission($permissionsData,'groups','edit');
$checkDelete = checkPermission($permissionsData,'groups','delete');
...
$allGroupsNumber = getRows("SELECT id FROM `groups` $filter");

/* 1. Xác định số lượng bản ghi trên 1 trang */
$perPage = _PAGE; /* Mỗi trang có 3 bẳn ghi */

/* 2. Tính số trang */
$maxPage = ceil($allGroupsNumber / $perPage);
...
$listAllGroup = getRaw("SELECT * FROM `groups` $filter ORDER BY groups.createAt DESC LIMIT $offset,$perPage");
    ...
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="row">
            <form action="" class="d-flex mb-3" method="get">
                <?php if($checkAdd):?>
                <div class="col-lg-2">
                    <a href="<?php echo getLinkAdmin($module, 'add') ?>" class="btn btn-primary">Thêm nhóm tài khoản</a>
                </div>
                <?php endif; ?>
        ...
        <div class="card">
            ...
            <div class="card-header">
                <h5 class="card-title">Danh sách nhóm tài khoản</h5>
            </div>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td>
                                        <a href="<?php echo getLinkAdmin($module, 'permission', ['id' => $item['id']]) ?>" class="btn btn-sm btn-outline-info">Phân quyền</a>
                                    </td>

                                    <td><?php echo (!empty($item['createAt'])) ? getDateFormat($item['createAt'], 'd/m/Y H:i:s') : false; ?></td>
