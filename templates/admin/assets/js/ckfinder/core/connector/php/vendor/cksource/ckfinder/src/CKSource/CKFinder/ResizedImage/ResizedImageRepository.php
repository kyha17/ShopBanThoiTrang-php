...
class ResizedImageRepository
{
    ...
    public function getResizedImage(
        ...
        if (!$this->acl->isAllowed($sourceFileResourceType->getName(), $sourceFileDir, Permission::IMAGE_RESIZE_CUSTOM)
            ...
            throw new UnauthorizedException('Provided properties is not allowed in images.sizes configuration');
        }
    ...
    /**
     * Checks if the provided image properties is allowed in the configuration.
     *
     * This is checked when `Permission::IMAGE_RESIZE_CUSTOM`
     * is not allowed in the source file folder.
     *
     * @return bool `true` if the provided properties is allowed in the configuration
     */
    ...
    /**
     * Returns the properties name defined in the configuration, where width
     * or height are equal to those given in parameters.
     *
     * Resized images keep the original image aspect ratio.
     * When an image is resized using the properties from the configuration,
