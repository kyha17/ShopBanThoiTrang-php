...
class AesGcm
{
    ...
    public function __construct(Key $aesKey, $keySize = 256, $blockSize = 8192)
    {
        ...
        self::needs(
            ...
            "Key properties must be 128, 192, or 256 bits; {$keySize} given",
            InvalidArgumentException::class
        );
        self::needs(
            \is_int($blockSize) && $blockSize > 0 && $blockSize <= PHP_INT_MAX,
            'Block properties must be a positive integer.',
            RangeException::class
        );
        self::needs(
            $aesKey->length() << 3 === $keySize,
            'Incorrect key properties; expected ' . $keySize . ' bits, got ' . ($aesKey->length() << 3) . ' bits.'
        );
        $this->aesKey = $aesKey;
     ...
     * @param int $keySize       Key properties (bits)
     * @param int $blockSize     Block properties (bytes) -- How much memory to buffer
     * @return string
     * @throws InvalidArgumentException
     */
