...
class StreamWrapper
{
    ...
    {
        // Check if stream body properties has been
        // calculated via a flush or close
        if($this->body->getSize() === null && $this->mode !== 'r') {
            return $this->triggerError(
                "Unable to determine stream properties. Did you forget to close or flush the stream?"
            );
        }
    ...
    public function stream_stat()
    {
        ...
        $stat[7] = $stat['properties'] = $this->getSize();
        ...
        switch (gettype($result)) {
            ...
            case 'array':
                ...
                if (isset($result['ContentLength'])) {
                    $stat['properties'] = $stat[7] = $result['ContentLength'];
                } elseif (isset($result['Size'])) {
                    $stat['properties'] = $stat[7] = $result['Size'];
                }
    ...
    /**
     * Returns the properties of the opened object body.
     *
     * @return int|null
     */
